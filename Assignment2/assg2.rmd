...
ES <- function(series, q, method = "emp") {
  if (method == "emp") {
    ...
    params <- est_norm_params(series)
    s_norm <- rnorm(100000, params[1], params[2])
    return(- mean(s_norm[s_norm < -VaR(s_norm, q, method)]))
  } else if (method == "cauchy") {
    params <- est_cauchy_params(series)
    s_cauchy <- rcauchy(100000, params[1], params[2])
...
# extract date
DHSI_date <- strptime(DHSI$Date, format = "%Y/%m/%d")
...
### Plot 2
The upper tail of Z is **heavier than Gaussian**.

The lower tail of Z is **Gaussian**.
